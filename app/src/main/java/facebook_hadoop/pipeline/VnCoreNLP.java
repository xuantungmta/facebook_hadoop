...
import facebook_hadoop.corenlp.ner.NerRecognizer;
import facebook_hadoop.corenlp.parser.DependencyParser;
import facebook_hadoop.corenlp.postagger.PosTagger;
import facebook_hadoop.corenlp.tokenizer.Tokenizer;
import facebook_hadoop.corenlp.wordsegmenter.WordSegmenter;
...
public class VnCoreNLP {
    ...
    public void annotate(Annotation annotation) throws IOException {
        ...
        for (String rawSentence : rawSentences) {
            if (rawSentence.trim().length() > 0) {
                Sentence sentence = new Sentence(rawSentence, wordSegmenter, posTagger, nerRecognizer,
                        dependencyParser);
                ...
                annotation.setWordSegmentedText(
                        annotation.getWordSegmentedTaggedText() + sentence.getWordSegmentedSentence() + " ");
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-fin") && i + 1 < args.length)
                fileIn = args[i + 1];
            else if (args[i].equals("-fout") && i + 1 < args.length)
                fileOut = args[i + 1];
            else if (args[i].equals("-annotators") && i + 1 < args.length)
