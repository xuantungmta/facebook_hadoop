package facebook_hadoop.corenlp.postagger;
...
public class PosTagger {
    ...
    public List<facebook_hadoop.pipeline.Word> tagSentence(String sentence) throws IOException {
        List<facebook_hadoop.pipeline.Word> output = new ArrayList<>();
        ...
        for (int i = 0; i < marmotSentence.size(); ++i) {
            ...
            facebook_hadoop.pipeline.Word word = new facebook_hadoop.pipeline.Word((i + 1),
                    marmotSentence.getWord(i).getWordForm(),
                    (String) token_lemma_tags.get(1));
            output.add(word);
