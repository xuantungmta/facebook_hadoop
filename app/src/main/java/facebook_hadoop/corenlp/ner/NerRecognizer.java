package facebook_hadoop.corenlp.ner;
...
import facebook_hadoop.corenlp.wordsegmenter.Vocabulary;
import facebook_hadoop.pipeline.LexicalInitializer;
import facebook_hadoop.pipeline.Word;
...
public class NerRecognizer {
    ...
    public NerRecognizer() throws IOException {
        ...
        if (!new File(modelPath).exists())
            throw new IOException("NerRecognizer: " + modelPath + " is not found!");
    ...
    private NLPNode[] toNodeArray(List<Word> sentenceWords) {
        ...
        for (int i = 0; i < sentenceWords.size(); i++) {
            Word word = sentenceWords.get(i);
            nlpNodes[i + 1] = new NLPNode(word.getIndex(), word.getForm(), word.getForm(), addLabelForPOSTag(word),
                    new FeatMap());
